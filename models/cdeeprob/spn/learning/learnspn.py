...
from cdeeprob.utils.random import RandomState, check_random_state
from cdeeprob.spn.structure.leaf import Leaf
from cdeeprob.spn.structure.node import Node, Sum, Product, assign_ids
from cdeeprob.spn.learning.leaf import LearnLeafFunc, get_learn_leaf_method, learn_naive_factorization
from cdeeprob.spn.learning.splitting.rows import SplitRowsFunc, get_split_rows_method, split_rows_clusters, split_rows_clusters_credal
from cdeeprob.spn.learning.splitting.cols import SplitColsFunc, get_split_cols_method, split_cols_clusters
...
def learn_spn(
    ...
    while tasks:
        ...
        if op == OperationKind.REM_FEATURES:
            ...
            dists, doms = [distributions[s] for s in task.scope], [domains[s] for s in task.scope]
            nans = task.data[np.isnan(task.data).any(axis=1), :]
            if not nans.any():
